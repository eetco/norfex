...
class WizardFormsController extends AppController
{
		private $listname, $amount ;
		public $codev ;
		var $components = array( 'Wizard' ) ;
		var $helpers = array( 'Wizard' ) ;
		var $uses = array( 'WizardForm', 'Currency', 'Country', 'Sender', 'NewCustomer',
				'Receiver', 'Summary', 'Payment', 'Orders' ) ;



		function beforeFilter()
		{
				if ( $this->action != 'test' ) $this->Wizard->steps = array( 'general', 'sender',
								'verification', 'receiver', 'summary', 'payment', array( 'bank' => array( 'bank_confirmation',
								'bank_receipt', 'finish' ), 'cash' => array( 'cash_confirmation', ) ) ) ;
		...
		function wizard( $step = null, $transactionId = null )
		{
			...
			//	print_r( $this->Session->read( 'Wizard.WizardForms.verification.NewCustomer.code' ) ) ;

				//print_r($this->Session->read('Config.receiver_id'));
			//print_r( $this->Session->read());

				// echo $this->Session->read('sender.last_Name');
				// print_r( $this->Session->read('Wizard.WizardForms.payment.Payment.paymentMethod'));
				//print_r( $this->Session->read('Wizard.WizardForms.general.WizardForm.totalamount'));


				$postalcodev = $this->Session->read( 'Wizard.WizardForms.sender.Sender.postal_code_sender' ) ;
				$daycodev = $this->Session->read( 'Wizard.WizardForms.sender.Sender.dob_sender.day' ) ;
                echo $codev = Security::hash( 33, 'md5', true ) ;
			...
			//	print_r( $codev ) ;
				...
				switch ( $step )
				{
						...
						case 'receiver':
								...
								$countryname = $this->Session->read( 'Wizard.WizardForms.general.country_receiver' ) ;
		...
		function _processPayment()
		{
				...
				if ( $this->Payment->validates() )
				{
					...
					$fields['sender']['Sender']['verification_code'] = $this->Session->read( 'Wizard.WizardForms.verification.NewCustomer.code' );
						...
						$order = array( 'Orders' => array( 'sender_id' => $receiver_id, 'receiver_id' =>
                        
								$sender_id, 'currency_sender' => 'SEK', 'amount_sender' => $this->Session->read
                                
								( 'Wizard.WizardForms.general.WizardForm.totalamount' ), 'fee' => $this->
                                
								Session->read( 'Wizard.WizardForms.general.WizardForm.expeditionfee' ),
                                
								'order_id' => $code['code1'], 'payment_status' => 'notpaid',
						...
						//$this->Session->Delete( 'Wizard.WizardForms.payment.Payment.paymentMethod' ) ;
						$this->Session->write( 'Wizard.WizardForms.payment.Payment.paymentMethod', $peyment ) ;
						$s = $this->Session->read( 'Wizard.WizardForms.payment.Payment.paymentMethod' ) ;
						echo $s ;
						if ( $s == 'cash' )
						{
