...
//加密
    function   encrypt($content, $offset, $key, $times) { //times为加密轮数
        $a1 = array(0,0,0,0);
        $a2 = array(0,0,0,0);
...
//byte[]型数据转成int[]型数据
    function   byteToInt($content, $offset) {
        ...
        $result =array(); //除以2的n次方 == 右移n位 即 content.length / 4 == content.length ＞＞ 2
...
//int[]型数据转成byte[]型数据
    function  intToByte($content, $offset) {
       $result = array(); //乘以2的n次方 == 左移n位 即 content.length * 4 == content.length ＜＜ 2
...
function   KingKey_HOTP_GetSpaceDayBetweenLastTime($SpaceMovingValue, $IntervalInSecond)
{
        ...
        $TotalDays =(int)($lTotalSeconds / 86400); //一天86400秒
 ...
 function  checkpassword($pInitCB,$pPassword)
{
       $pInitCB=trim($pInitCB);
        ...
        else if(($iLastTimebettwenDays > 30) && ($iLastTimebettwenDays <= 90)) // 1~3个月
        {
                $iTimeWindow = 10;
        }
        else if(($iLastTimebettwenDays > 90) && ($iLastTimebettwenDays <= 180)) // 3~6个月
        {
                $iTimeWindow = 15;
        }
        else if(($iLastTimebettwenDays > 180) && ($iLastTimebettwenDays <= 360)) // 6~12个月
        {
                $iTimeWindow = 30;
        }
        else // 12个月以上
        {
			...
			else if($stHotpInfo->OTPChangeTime==0)
			{
					 $iTimeWindow = $iTimeWindow;
			}
			else
		...
		if($stHotpInfo->OTPChangeTime==0)
		{
			for($i=$stHotpInfo->LastLoginTotalMovingValue+1 ;$i<=$stHotpInfo->LastLoginTotalMovingValue + 200 ;$i++)
        	 {
        	   
				 
				if( $this->CheckOTPpassword($stHotpInfo->Key,$OTPLen,$pPassword,$i)==1)
                {
                      
                        $stHotpInfo->LastLoginTotalMovingValue = $i;
 ...
 function  passwordsyn($pInitCB,$pPassword)
{
       $pInitCB=trim($pInitCB);
		...
		if($stHotpInfo->OTPChangeTime==0)
		{
			for($i=$stHotpInfo->LastLoginTotalMovingValue+1 ;$i<=$stHotpInfo->LastLoginTotalMovingValue + 300 ;$i++)
        	 {
...
function GetmMatrixpassword($key,$Challenge)
{
   ...
   for($i=0;$i<strlen($Challenge);$i++)
   {
    ...
    return  substr($this->GetHOTP8Len( $newkey,pack("H*",$str)),5,3);      

}

function Checkmatrixpwd($sninfo,$in,$password)
{
	$key=new matrixclass();
	
	return $key->matrixCheck($sninfo,$in,$password);
...
function matrix_GetInfoFromCB($pInitCB)
    {
...
function matrixCheck($sninfo,$Challenge,$Response)
	
	{
		
		if($this->matrix_CheckCB($sninfo)!=1)
		{
			return "-1";
		}
		...
		$infoc=$this->matrix_GetInfoFromCB($sninfo);
		
		$si=new seamoonapi();
		$sg=new stringchange();

		if(strcmp($si->GetmMatrixpassword($sg->bytetoString($infoc->Key),$Challenge ),$Response)==0)
        { 					 
                return "1";
        }
        else
